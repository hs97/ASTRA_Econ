...
from utils import evaluate_ran, analyze_rule_attention_scores, evaluate, evaluate_test
...
def astra(args, logger):
    ...
    # A list holding all the results
    ...
    logger.info(train_dataset.data['labels'])
    ...
    pseudodataset.report_stats('labels')
    ...
    logger.info(results['supervised_student_dev'])
    ...
    for iter in range(args.num_iter):
        ...
        prev_min = min([x['perf'] for x in dev_res_list])
        if dev_res['perf'] < prev_min:
            logger.info("Improved dev performance from {:.2f} to {:.2f}".format(prev_min, dev_res['perf']))
    ...
    best_dev_epoch = len(all_dev) - np.argmin(all_dev[::-1]) - 1
    best_test_epoch = len(all_test) - np.argmin(all_test[::-1]) - 1
    logger.info("BEST DEV {} = {:.3f} for epoch {}".format(args.metric, all_dev[best_dev_epoch], best_dev_epoch))
    logger.info("FINAL TEST {} = {:.3f} for epoch {} (min={:.2f} for epoch {})".format(args.metric,
                                                                                       all_test[best_dev_epoch], best_dev_epoch, 
                                                                                       all_test[best_test_epoch], best_test_epoch))
