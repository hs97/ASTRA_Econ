...
def get_results(resfolder):
    ...
    if 'teacher_dev_iter' in res:
        ...
        teacher_all_dev_mse = [x['mse'] for x in res['teacher_dev_iter']]
        teacher_all_test_mse = [x['mse'] for x in res['teacher_test_iter']]
        ...
        teacher_all_dev, teacher_all_test, teacher_all_dev_mse, teacher_all_test_mse, teacher_perf_str = [0], [0], [0], [0], ''

    student_all_dev = [x['perf'] for x in res['student_dev_iter']]
    student_all_test = [x['perf'] for x in res['student_test_iter']]
    student_all_dev_mse = [x['mse'] for x in res['student_dev_iter']]
    student_all_test_mse = [x['mse'] for x in res['student_test_iter']]
    ...
    return {
        ...
        'teacher_dev_mse': teacher_all_dev_mse,
        'teacher_test_mse': teacher_all_test_mse,
        'student_dev_mse': student_all_dev_mse,
        'student_test_mse': student_all_test_mse,
        'metric': metric
    }
...
def summarize_results(basefolder, dataset):
    ...
    with open(savefile, 'w') as f:
        ...
        for seedfolder in seedfolders:
            ...
            best_student_iter = np.argmin(res['student_dev_perf'])
            best_teacher_iter = np.argmin(res['teacher_dev_perf'])
            ...
            best_students_acc.append(res['student_test_mse'][best_student_iter])
            best_teachers_acc.append(res['teacher_test_mse'][best_teacher_iter])
        ...
        best_student_iter = np.argmin(avg_student_dev_perf)
        best_teacher_iter = np.argmin(avg_teacher_dev_perf)
